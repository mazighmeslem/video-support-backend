...
// Endpoint to get TURN credentials for client
  ...
  .then(function(iceServers){
    ...
    pc.ontrack=function(e){
      ...
      v.onloadedmetadata=function(){
        v.play().then(showVideo).catch(showVideo);
      };
      setTimeout(function(){
        if(v.srcObject) showVideo();
      },3000);
    };
...
function doWebRTC(){
  return fetch(B+'/turn-credentials')
  .then(function(r){ return r.json(); })
  .then(function(iceServers){
    pc=new RTCPeerConnection({ iceServers: iceServers });
